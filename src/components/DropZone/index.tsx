...
const DropZone: FC<{ update: (file: any) => void }> = ({ update }) => {
	...
	const renderContent = () => {
		if (selectedFile?.name) {
			return <div className="flex flex-col items-center">{selectedFile?.name}</div>
		}
		return acceptedFiles.length > 0 ? (
			<div className="flex flex-col items-center">{acceptedFiles[0].name}</div>
		) : (
			<div className="flex flex-col items-center">
				<p>
					<b>Choose a file</b> or drag and drop your file
				</p>
			</div>
		)
	}
	...
	const renderItems = () => {
		...
		if (!medias.length)
			return (
				<div className="text-secondary text-sm col-span-5">
					No items found, Start uploading and it will start apperating here
				</div>
			)
	...
	return (
		<>
			<div
				...
				className="border-2 border-dashed border-secondary px-5 py-16 rounded-xl bg-stroke flex flex-col justify-between items-center relative overflow-hidden"
			>
				<input {...getInputProps()} />
				...
				<svg width="70" height="70" viewBox="0 0 96 96" fill="none" xmlns="http://www.w3.org/2000/svg">
					<g clipPath="url(#clip0_200_2013)">
						...
						/>
						<path
							d="M55.4219 12.7852V35.0508H77.6875"
							stroke="#262877"
							strokeWidth="4"
							strokeLinecap="round"
							strokeLinejoin="round"
						/>
						<path
							d="M39.0938 55.832L48 46.9258L56.9062 55.832"
							stroke="#262877"
							strokeWidth="4"
							strokeLinecap="round"
							strokeLinejoin="round"
				...
				<p className="text-secondary text-lg mt-6">{renderContent()}</p>
				{isDragActive && (
...
const Item: FC<{ name: string; preview?: string; onClick: any }> = ({ name, preview, onClick }) => {
	return (
		<div className=" rounded-md bg-[#EEF2FE] cursor-pointer hover:bg-[#c5d0f1]" onClick={onClick}>
			<div
				className="text-center p-2 m-[2px] h-[80px] text-sm text-secondary rounded-md bg-[#F7F9FF] flex justify-center items-center bg-cover"
