...
const Apply = ({ children }: { children: React.ReactNode }) => {
	...
	return (
		<div className={twMerge("flex-1 text-5x flex flex-col items-center p-6", fullPage && 'p-0')}>
			{
				!fullPage && <div className={twMerge('flex-1 items-center flex', steps.length && 'hidden')}>
			...
			<div id="roooot" className={twMerge('max-w-[min(80%,900px)] w-full mt-9 relative', !steps.length && 'hidden', fullPage && "flex-col flex mt-0 max-w-full h-full flex-1 justify-center items-center")}>
